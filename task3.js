...
// let is block-scoped, that means its only accessible inside block(checkPrint) function, So seceret belongs to funcion scope(block scope), its does not exist in the global scope, if we want to print secret in global scope means we need to call the function and store it in a variable in global scope and that fuction should return the secret variable that time only we can get a secret variable's value.
